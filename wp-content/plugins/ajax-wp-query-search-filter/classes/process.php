...
if(!class_exists('awpqsfprocess')){
	class awpqsfprocess{
	   ...
	   function get_ajaxwqsf_taxo($id, $gettaxo){
		...
		function relevanssi_taxonomy($arg){
	   		$taxonomy = '';
	   		$terms = '';
	   		foreach ($arg as $value) {
	   			if (is_array($value)){
	   				if (array_key_exists('taxonomy', $value) && array_key_exists('terms', $value) && !array_key_exists('operator', $value)) {
			...
			$result = array('taxonomy' => $taxonomy, 'terms' => $terms);
			return $result;
	   }
		...
		//	if(isset($formid) && wp_verify_nonce($nonce, 'ajaxwpsfsearch')){
					...
					$relevanssi_query = $this->relevanssi_taxonomy($tax_query);
					...
					$args = array(
								...
								'tax_query' => $tax_query,
								'taxonomy' => $relevanssi_query['taxonomy'],
								'terms' => $relevanssi_query['terms'],
								's' => esc_html($keyword),
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				$query->the_post();
					...
					$html .= '<span class="entry-title" style="font-size:inherit;"><a href="'.get_permalink().'" rel="bookmark">'.get_the_title().'</a></span>';
					$html .= '</header>';
					$html .= '<div class="entry-summary">'. do_shortcode('[pods name="cours" id="'.get_the_id().' " template="excerpt"]') .'</div></article>';
			...
			$html .= $apiclass->ajax_pagination($pagenumber,$query->max_num_pages, 4, $id);
					...
					$html .= __( 'Aucun résultat', 'AjWPQSF' );}
				}
